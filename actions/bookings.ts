...
export async function createBooking(formData: FormData) {
  ...
  const { data: overlappingBookings } = await supabase.from('bookings').select('id').eq('room_id', roomId).eq('status', 'confirmed').or(`check_in.lte.${checkOut},check_out.gte.${checkIn}`).not('check_in', 'gte', checkOut).not('check_out', 'lte', checkIn)
